defmodule GossipWorker do
    ...
    def handle_cast({:sendMsg, pid, msg, is_self}, state) do
        ...
        #if(rand_pid!= nil && Process.alive?(Process.whereis(:"#{rand_pid}"))) do
            if(count >= 0) do
                if(!is_self) do
                    if(count > 0) do
        ...
        # end
        #GossipWorker.sendMessage(:"#{pid}", msg, true)
        #state = Map.put(state, :numNodes, length(new_list))
        {:noreply, state}
    end
